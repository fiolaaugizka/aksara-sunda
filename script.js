...
// =====================
const quizSound = document.getElementById("quizSound");
const muteQuizBtn = document.getElementById("muteQuizBtn");

let isMuted = false;

muteQuizBtn.addEventListener("click", () => {
  isMuted = !isMuted;
  quizSound.muted = isMuted;
  muteQuizBtn.textContent = isMuted ? "🔇" : "🔊";
});
...
if (konsonanKey) {
  ...
  if (konsonan) {
    ...
    // simpan wrapper ini untuk rarangken
...
if (vokalSetelah && vokalMap[vokalSetelah] && lastKonsonanWrapper) {
  ...
  // reset agar tidak menempel ke huruf berikutnya
